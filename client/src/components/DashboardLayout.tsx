...
import { BadgeDollarSign, BriefcaseBusiness, ChartNoAxesCombined, FileBarChart2, Landmark, LogOut, PanelLeft, ReceiptText, Scale, ShieldCheck, WalletCards } from "lucide-react";
  ...
  return <>
    <Sidebar collapsible="icon" className="border-r border-slate-800 bg-[#10243B] text-slate-100">
      <SidebarHeader className="h-[76px] border-b border-slate-700/70 px-3 py-0 justify-center">
        <div className="flex items-center gap-3 w-full"><button onClick={toggleSidebar} className="rounded-lg p-2 text-slate-300 hover:bg-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-300" aria-label="Thu gọn thanh điều hướng"><PanelLeft className="h-4 w-4" /></button>{!isCollapsed && <div className="min-w-0"><p className="font-serif text-base font-semibold leading-none tracking-wide text-white">VERITAS</p><p className="mt-1 text-[10px] uppercase tracking-[0.16em] text-teal-200">Finance Desk</p></div>}</div>
      </SidebarHeader>
    ...
    <SidebarInset className="min-w-0 bg-[#F7F9FC]">
      {isMobile && <header className="sticky top-0 z-40 flex h-14 items-center gap-3 border-b border-slate-200 bg-white/95 px-3 backdrop-blur"><SidebarTrigger className="rounded-lg" /><div><p className="text-sm font-semibold text-slate-800">{current.label}</p><p className="text-[10px] uppercase tracking-widest text-slate-500">Veritas Finance</p></div></header>}
