...
export default function PeriodClosePage() {
  ...
  const canApprovePeriod = access.data?.canApprovePeriod ?? false;
  const canLockPeriod = access.data?.canLockPeriod ?? false;
  const canReopenPeriod = access.data?.canReopenPeriod ?? false;
  ...
  const stateAction = () => {
    ...
    if (status === "pending_approval" && canApprovePeriod) approve.mutate({ periodKey });
    if (status === "approved" && canLockPeriod) lock.mutate({ periodKey });
    if (status === "locked" && canReopenPeriod) {
